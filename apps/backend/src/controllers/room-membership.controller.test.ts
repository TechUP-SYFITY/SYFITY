...
describe('RoomMembershipController', () => {
  it('신규 참여는 201과 영속 Room 정보만 반환한다', async () => {
    ...
    await expect(
      controller.createMembership(request, { inviteCode: 'ABC123' }, created),
    ).resolves.toEqual({
      success: true,
      data: {
        room: {
  ...
  it('기존 참여 이력 복원은 기본 200 응답을 반환한다', async () => {
    const createMembership = vi.fn().mockResolvedValue({ room, isNewMembership: false });
    const created = vi.fn();
    const controller = new RoomMembershipController({ createMembership });

    await controller.createMembership(request, { inviteCode: 'ABC123' }, created);
